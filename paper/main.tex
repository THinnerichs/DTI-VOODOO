...
connections and a normalization scheme \cite{DeepGCN2019,
  DeeperGCN2020} as shown in Supplementary Figure 3.  The residual
...
over MolTrans ($t=36.6$), DeepDTI ($t=40.5$), DeepDTA ($t=36.6$), and
DeepConv-DTI ($t=40.5$) on the BioSnap dataset, and significantly
($p<10^{-4}$, one-sided t-test) over DTINet ($t=20.8$) and DTIGEMS+
($t=18.0$) on the Yamanishi dataset; DTI-CDF performs significantly
better than \name{} ($p=0.02$, one-sided t-test). % As
                                % we compare \name{} to 4 different
                                % methods over BioSnap and 3 over
                                % Yamanishi dataset, we add a
                                % Bonferroni correction by multiplying
                                % the $p$-values by 4 and 3,
                                % respectively, leading to $p<0.0002$
                                % ($\alpha=0.01$) for all previous
                                % comparisons except DTI-CDF.
We further perform another one-tailed t-test to compare $MicroAUC_p$
performance. We find that \name{} improved significantly ($p <
10^{-4}$) over MolTrans($t=21.2$), DeepDTI ($t=31.0$), DeepDTA
...
methods. Several methods achieve a higher MacroAUC than \name{}, in
