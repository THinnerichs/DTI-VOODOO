...
\todo[inline]{Looking at the code, I wonder if what is implemented is
  actually a ANN; it does not look as if the weights are shared
  between the two networks, so looks more like a twin or half-twin
  network? Siamese would be sim(f(e1),f(e2)) but the implementation
  seems to be sim(f(e1), g(e2)).}
...
dimension.  The precomputed embeddings are first processed by a
learnable feature transformation (LFT) network, a fully connected
neural network layer which takes as input an embedding and outputs a
representation of a particular size. An example structure for both
types of features can be found in Figure~\ref{fig:HalfTwinNetwork}. We
use this LFT to reduce the eventual representation size for drugs and
proteins separately.
...
similar dimensionality for varying sizes of inputs; this allows for a
...
generated features can be used for a variety of other tasks; for
example, the ontology LFT can be reused for a variety of DL2vec-based
features with respect to other ontologies and hypotheses. We follow
the results of \textit{DL2vec} showing that the activation function
$\sigma := \mathrm{LeakyReLU}$ leads to improved performance compared
...
We construct a PPI graph with proteins as nodes and protein-protein
...
utilizing graph convolutional steps, calculating the similarity of
drug and protein representation, and thus eventually predicting
whether there is an interaction. The full workflow scheme is depicted
in figure~\ref{fig:ModelWorkflow}
