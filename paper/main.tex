...
\textit{MacroAUC} and $MicroAUC_p$\todo{which is it, macro or micro?},
...
stratification\todo{Stratification may not be the right word here; do
  you mean ``sub-sampling of negatives''?} for the training process
...
\cite{}\todo{add}, we reevaluated DeepDTI \cite{}\todo{add}, DeepDTA
\cite{}, DeepConv-DTI \cite{} and MolTrans itself on BioSnap\todo{What
...
schemes for predicting drug--target interactions. This was motivated
by our finding that a na\"ive predictor can achieve a performance only
slightly below state of the art drug--target prediction methods.
