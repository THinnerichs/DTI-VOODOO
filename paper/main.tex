...
% made. 
Identifying drug--target interactions (DTIs) is one of the initial steps in drug discovery and screening for disease treatment and thus helps to mitigate the the costs and risks of drug development.  Further, it forms a generalization of both drug repurposing and repositioning, finding novel functions for already approved and extensively analyzed drugs with well explored properties. Inferring and deriving from those existing properties and interactions can help to analyze and identify novel drugs for known targets, leading to desirable therapeutic effects. Moreover, while \textit{in vitro} DTI prediction requires the availability of all substances, computational, \textit{in silico} DTI predictors interpret and anticipate millions of interactions within short time.
...
methods face the challenge to predict whether a chemical structure binds to a
...
accurate prediction) but also properties namely in which celltypes and anatomical structures a protein is expressed. Top-down
...
molecular information are directly predicting whether two molecules
...
We further investigate the influence of the more specific interaction types \textbf{inhibition} and \textit{activation} within STITCH database on the performance of \name{}. The results are summarized in Supplementary table 3, where we find no difference in comparison to the complete dataset.
...
information about protein--protein interactions. Moreover, as the protein--protein 
interaction relation is not transitive (protein $P_1$ interacting with $P_2$, 
and $P_2$ relating with $P_3$ does not imply $P_1$ interacting
or influencing $P_3$), we can only infer from the direct neighborhood of each protein within the PPI graph. 
Our results (Table
\ref{tab:Results}) confirm the first hypothesis and demonstrate that
...
propagation over the interaction network. While the exploitation of ontology based features over PPI graphs was already examined in \citet{Trebacz2020} for gene expression analysis, our work is the first to investigate and apply this technique to DTI prediction. Further, \name{} is the first work to combine both ``bottom-up'' and ``top-down'' features.  
...
entities. We have demonstrated that even the newly proposed ``na\"ive'' classifier that
...
As the target-based predictive power of \name{} improves significantly over other methods, we utilized our model to predict novel drug classes for protein families. We therefore collected the 2nd level ATC (Anatomical Therapeutic Chemical) groups for each drug and all InterPro \cite{Interpro2020} top-level families for each protein. Utilizing the STITCH interactions, we followed a protein split within each InterPro family by predicting over all available drugs. We eventually normalized the number of novel interactions per class by the amount of proteins within the respective family. A heatmap showing the results of this analysis can be found in Supplementary figure 5. \dots \todo{Roberts class analysis}.
