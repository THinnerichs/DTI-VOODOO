...
We first perform a cross-validation over proteins and validate our
results. Cross-validation over proteins aims to evaluate how the model
performs when tasked to identify drugs that may target a ``novel''
protein, i.e., one not seen during training.
\todo[inline]{Describe/add the CV results here.}

Second, we use the model trained on STITCH and evaluate the model on
the Yamanishi benchmark dataset\todo{This needs another sentence
  stating how you made sure there was no overlap in data.}.

\todo[inline]{Maybe remove microAUC here, introduce in next section?}
...
\todo[inline]{Move to caption of table.}
...
(\textit{OntoPred}) is showing significantly\todo{significance test
  and p value? otherwise remove} better performance on STITCH compared
to using only molecular features. We also observe that only the model
using ontology-based features results in significantly\todo{p-val}
increased performance when incorporating the PPI graph.  This increase
can be observed with different graph neural network
architectures\todo{data not in the table}. While the {\em GCNConv} and
...
removed\todo{is this still a graph model?}. This pruned network, with
no information on the protein--protein interactions, reached very
similarly results as the original \textit{OntoPred} model\todo{data
  not shown; and details missing from Methods} and showed no
improvement.

\todo[inline]{Not clear, need to reformulate:}
...
As the inclusion of graph information significantly\todo{p val}
...
consequently, this na\"ive predictor does not predict any useful
information about interactions between drugs and proteins.
...
% Note, that this baseline predictor is not applicable for a protein split CV, as the amount of interactions is unknown. 
For each fold, we gradually increase $k$, eventually saving the best
performance for each fold. Using the full dataset, drug--target split,
and drug split, we obtain \todo[inline]{merge table results in
  text}\todo{are these micro or macro AUC?}.
...
The baseline predictor shows significantly\todo{p-val} higher
performance than a random classifier on both datasets, with better
results on the Yamanishi benchmark. 
...
previously evaluated on the Yamanishi benchmark dataset. These
state-of-the-art methods approaches include  DTIGEMS+
...
hyperparameters and fold among for cross-validation, measuring their
respective AUROC score. We eventually assess each model, by performing
a protein-wise cross-validation determining (macro) AUROC and
$MicroAUC_p$. For this we allow stratification for the training
process, but not for the validation and testing phase, as real world
scenarios are rather imbalanced. This however, only impacts the not
