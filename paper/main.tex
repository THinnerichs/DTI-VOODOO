...
introduced by \citet{Yaminishi2008} consisting of 1923 interactions between 708 drugs and 1512 proteins.
...
\todo[inline]{Looking at the code, I wonder if what is implemented is
  actually a 
   ANN; it does not look as if the weights are
...
We use two half-twin neural networks to combine the molecular and
...
protein-wise cross-validation determining (macro) AUROC and
...
real world applications of these models would have to deal with
imbalanced data.

\todo[inline]{Merge the tables; add \name model to table to allow
  DIRECT comparison:}
