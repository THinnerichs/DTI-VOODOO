...
\title[short Title]{\name: Combining Bottom-up and top-down approaches through graph learning over interaction networks for drug-target-interaction prediction}git
...
\begin{table}[!tpb]
	...
	\caption{Results for \name on STITCH and Yamanishi dataset evaluated with a 5-fold cross-validation. MolPred and OntoPred are the predictors for molecular and ontology based features, respectively. We hereby denote the molecular feature based predictor with \textit{MolPred}, while abbreviating the ontology based, top-down predictor with \textit{OntoPred}.}
...
Table \ref{tab:STITCHresults} shows the results of these experiments.
...
using ontology-based features results in
increased performance when incorporating the PPI graph. This increase
can be observed with different graph neural network
architectures and configurations. While the {\em GCNConv} and
{\em GENConv} architecture already shows some minor improvement, the
...
removed, resulting in a non--graph model. This pruned network, with
no information on the protein--protein interactions, reached very
similar results to the original \textit{OntoPred} model\todo{data
...
The na\"ive predictor shows great performance on Yamanishi dataset, with substantial gain in comparison to a random predictor on both datasets. In the following we will denote this nai\"ve predictor as a baseline predictor correlating its performance to state of the art models and \name.
