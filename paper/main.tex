...
To assess each model, we compute a variety of common metrics for binary classification. As the datasets are highly imbalanced, we chose the \textit{area under receiver operating characteristic}-score (AUROC) on training, validation and testing split. Therefor, we calculated the true positive rate (TPR), false positive rate (FPR) and precision score. We will further compute true positives (TP), false positives (FP), false negatives (FN) and finally true negatives (TN). 
We calculate the ROC by plotting the TPR against the FPR for various threshold settings for the model predictions. We eventually calculate the area under ROC curve utilizing trapezoidal approximations. We will refer to this measure as $MacroAUC$.
...
Note that drugs and proteins can be interchanged in this formulation, which we will denote with $MicroAUC_p$ and $MicroAUC_d$, respectively. Additionally, the $MicroAUC$ score may not be defined as, in some datasets some targets or drugs, respectively, do not have any interactions, leading to an infeasible $TPR$ and an undefined $AUROC$ score for that entity. For those entities we impute the $MicroAUC$ linearly, by using the accuracy for this subset. 


\subsection{Model}
Before introducing the models core components, we will clarify the previously mentioned notions of top-down and bottom-up. We will refer to features and methods, starting from the observable characteristics of either drug or protein inferring the likely molecular mechanisms as top-down. On the other hand, bottom-up refers to approaches deducing from even finer molecular properties, such as single functional groups or amino acid sequences for drugs and proteins, respectively. 
...
forcing a similarity between these embeddings. On the other hand we built a deep neural network for the molecular structure based features, also benefiting from the siamese network
architecture. Computing the similarity between two representations, allows for a variety of different methods.
However, we decided for the cosine similarity measure being invariant to scaling.

Therefore, the precomputed embeddings are run through a
regular, neural learnable feature transformation (LFT) network, which also reduces the
...
As the number of drug-targets are sparsely given with respect to the number of both drugs and proteins considered, the resulting training, validation and testing datasets are highly
imbalanced. As there are only $22.336$ links in
...
this ratio by introducing the following loss function with respect to binary cross-entropy:
\begin{equation}
...
% and
% ontology features, yielding the deep neural network transformers
% (DNNT) for both methods, and both drugs and proteins,
% individually. 
and add these to a protein interaction network as node features.
% neural network in which nodes representing interactions
% between proteins as node features, 
We then train this model in an end-to-end manner. % Notably, like in the
% pretraining phase, we rely on siamese neural networks, enforcing
% similar representations for fitting drug-protein pairs. 
...
%To obtain a prediction for a given drug-protein pair,
If 
...
\begin{figure}[!tpb]
	\centerline{\begin{tabular}{|p{2.2cm}|p{0.9cm}|p{0.75cm}|p{0.9cm}|p{0.75cm}|}
		...
		&\multicolumn{2}{c|}{without}&\multicolumn{2}{c|}{with}\\
		&AUROC&Micro AUC&AUROC&Micro AUC\\
		\hline
...
\end{tabular}}
\vspace{0.5cm}
This baseline predictor shows remarkable performance on both datasets, that 
