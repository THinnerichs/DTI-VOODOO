...
\title[\name{}]{\name{}: Combining Bottom-up and top-down approaches
  through graph learning over interaction networks for
  drug-target-interaction prediction} \author[Sample
\textit{et~al}.]{Tilman Hinnerichs\,$^{\text{\sfb 1,}*}$ and Robert
  Hoehndorf\,$^{\text{\sfb 2}}$}
\address{$^{\text{\sf 1}}$Department, Institution, City, Post Code, Country and \\
  $^{\text{\sf 2}}$Computational Bioscience Research Center, Computer,
...
knowledge gained from molecular properties. While methods tha3t rely on
...
\todo[inline]{CITE}\cite{}. Biological networks used for DTI prediction include
protein--protein interaction networks \todo[inline]{CITE}\cite{} and networks including
several other types of biological relations, including similarity
between represented entities \cite{Gottlieb2011}. Network-based DTI
...
\cite{Gillis2012}. Network-based methods have been applied successfully
...
prediction of protein functions \cite{Zitnik2017}, cancer drug response \cite{Liu2020} and drug--target affinity prediction \cite{GraphDTA2020}.
...
ontology-based machine learning method \cite{DL2vec2020} to encode phenotypic
...
them. \name{} is available as Free Software at \url{https://github.com/THinnerichs/DTI-VOODOO}.
...
We follow the results of \textit{DL2vec} \cite{DL2vec2020} and use as
...
\ref{fig:FullModelAllLayers}. Here
...
for that entity. As this is quite common for DTI datasets, we do not omit but impute the $MicroAUC$
interpolating linearly for those entities, by using the accuracy for this
subset:
...
Further, we choose AUROC over area under precision recall curve (AUPRC) as the latter is prone to data imbalance \cite{Jeni2013} and thus sub-sampling biases, undermining cross-approach evaluation.\todo[inline]{More Information? Right place?}
...
$MicroAUC_p$, see Section \ref{sec:evaluation}), and retrain them from scratch in a
...
predictive of drug--target interactions \todo[inline]{CITE}\cite{}; however, we do
...
determine indirect effects of molecular changes \todo[inline]{CITE}\cite{} and predict
drug--target interactions \todo[inline]{CITE}\cite{}. Our work relies on graph neural
...
work \cite{MolTrans2020}\cite{DeepConvDTI2019}, we find significant differences in predictive
