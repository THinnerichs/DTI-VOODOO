...
As DTI prediction is dependent on both drugs and proteins, there are multiple ways of determining training, validation and testing sets of pairs to evaluate each model. For cross-validation, we can perform the split over DTI pairs, over drugs and over proteins, respectively, where only in the latter two schemes unseen entities are shown to the model in the validation and testing phase. Additionally, these schemes have diverging expressiveness with respect to DTI prediction, as different information is known and leaked across training and testing phase. 
...
Note that drugs and proteins can be interchanged in this formulation. We
...
\begin{table*}[!tpb]
  \centering
	...
	\begin{tabular}{|l|p{1cm}|p{1cm}|p{1cm}|p{1cm}|p{1cm}|p{1cm}|p{1cm}|}
		...
		\hline
		&&\multicolumn{3}{c|}{(c) Yamanishi results}&\multicolumn{3}{c|}{(d) BioSnap results}\\
		Approach&Original&Original scheme&\multicolumn{2}{c|}{Protein split}&Original scheme&\multicolumn{2}{c|}{Protein split}\\
		
		&Splitting scheme&Macro AUC&Macro AUC&Micro $AUC_p$&Macro AUC&Macro AUC&Micro $AUC_p$\\
		\hline
		Naive predictor&DP pairs&$0.85$& --&--&$0.79$&-- &--\\
		...
		\name&Proteins&$0.84$&$0.84$&$\mathbf{0.84}$&0.85 & $\mathbf{0.85}$& $\mathbf{0.82}$\\
		\hline
	\end{tabular}\medskip
