...
public class MotifValidator {
  ...
  private static void parseMemeXML(MotifStats stats, XMLStreamReader cursor, FeedbackHandler feedback) throws XMLStreamException, AlphParser.AlphParseException {
    ...
    while (!(cursor.isEndElement() && cursor.getLocalName().equals("motifs"))) {
      ...
      tagAtrs(cursor, true, feedback, new Attr("id"), new Attr("name"), new Attr("alt", true), aMotifWidth, new Attr("sites"), new Attr("ic"), new Attr("re"), new Attr("llr"), new Attr("p_value", true), new Attr("e_value"), new Attr("bayes_threshold"), new Attr("elapsed_time"), new Attr("url", true));
      int motifWidth;
      try {
