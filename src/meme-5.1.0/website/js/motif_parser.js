...
HtmlMotifParser.prototype._html_data = function (data) {
  ...
  if (program == null || (program != "MEME" && program != "dreme")) {
    throw new MotifError("Unrecognised program, expected MEME or dreme.");
  }
  this.rating = 3;
  ...
  if (typeof alphabetData.symbols === "string") {
    if (alphabetData.symbols == "ACGT") {
      ...
      throw new MotifError("Unrecognised alphabet string \"" + alphabetData.symbols + "\".");
    }
...
XmlMotifParser.prototype.process_meme_doc = function (doc) {
  ...
  if (alphabet_id != null) {
    if (alphabet_id === "nucleotide") {
      ...
      throw new MotifError("MEME XML value /MEME/training_set/alphabet/@id is unrecognised \"" + alphabet_id + "\".");
    }
  } else {
    try {
  ...
  if (strands == "both" || strands == "forward" || strands == "none") {
    ...
    throw new MotifError("MEME XML contains an unrecognised strand type \"" + strands + "\".");
  }
  this.rating = 4;
  // get the background
...
XmlMotifParser.prototype.process_dreme_doc = function (doc) {
  ...
  if (norc_ele == null) {
    ...
    throw new MotifError("DREME XML contains an unrecognised value for the norc \"" + norc + "\".");
  }
  this.rating = 4;
  // get the background
