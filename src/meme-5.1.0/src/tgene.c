...
typedef struct options {
  ...
  char *fdesc;			// file containing description
...
static void output_html_file(
  ...
  // Output description
  jsonwr_desc_prop(json, "job_description", options->fdesc, options->desc);
...
static LOCUS_TABLE_T *initialize_locus_table(
  ...
  for (i=n_valid=0; i<n_loci; i++) {
    ...
    asprintf(&key, "%s:%ld-%ld", get_bed_chrom(reg_el), get_bed_start(reg_el), get_bed_end(reg_el));
...
static int update_link_table(
  ...
  for (i=first_link_index; i<n_links; i++) {
    LINK_T *link = links[i];
    asprintf(&key, "%s_%s:%ld-%ld", link_tss_id(link), link_re_chrom(link), link_re_start(link), link_re_end(link));
  ...
  for (locus_i=trans_i=0; locus_i<n_loci; locus_i++) {
	...
	transcript = &transcripts[tied_i];
	  ...
	  // Key is <transcript_id>_<locus>.
	  asprintf(&key, "%s_%s:%ld-%ld", get_transcript_id(transcript), get_locus_chrom(locus), get_locus_start(locus), get_locus_end(locus));
	  HASH_TABLE_ENTRY *hte = hash_lookup_str(key, ht_links);
	  if (! hte) {					// Add new link.
      ...
      asprintf(&key, "%s:%ld-%ld", get_locus_chrom(locus), get_locus_start(locus), get_locus_end(locus));
  ...
  for (link_i=0; link_i<n_links; link_i++) {
    ...
    asprintf(&key, "%s:%ld-%ld", link_re_chrom(link), link_re_start(link), link_re_end(link));
