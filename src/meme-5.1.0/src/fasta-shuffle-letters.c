...
enum Opts {OPT_ALPH, OPT_DNA, OPT_PROTEIN, OPT_RNA, OPT_KMER, OPT_COPIES,
  OPT_PRESERVE, OPT_SEED, OPT_LINE, OPT_TAG, OPT_HELP, OPT_VERSION};
...
/***********************************************************************
 Output a sequence
 ***********************************************************************/
static void output_seq(FILE *out, ALPH_T *alph, int line, int kmer, int copies, int preserve, char *tag, STR_T *id, STR_T *seq) {
  char *shuf, *seq2;
...
static size_t routine_seq_whitespace(OPTIONS_T *opts, STATE_T *state, char *buffer, size_t len) {
  ...
  for (i = 0; i < len; i++) {
    ...
    if (state->nl && buffer[i] == '>') {
      output_seq(state->out, state->alph, opts->line, opts->kmer, opts->copies, opts->preserve, opts->tag, state->id, state->seq);
      str_clear(state->id);
      str_clear(state->seq);
...
static void process(OPTIONS_T *opts, ALPH_T *alph, FILE *in_fh, FILE *out_fh) {
  ...
  if (state.routine != routine_find_start) {
    output_seq(out_fh, alph, opts->line, opts->kmer, opts->copies, opts->preserve, opts->tag, state.id, state.seq);
  }
  // clean up
