...
static void read_grid(CTX_T *ctx, bool is_pspm, const char *grid) {
  ...
  if (alph != NULL && alph_size_core(alph) != line_count) {
  ...
  if ((node = rbtree_find(kv_pairs, alpha_key)) != NULL) {
    ...
    if (errno != 0 || *end != '\0') {
      ...
      if (parser->fscope.alphabet != NULL) {
        ...
        switch (alen) {
          case 4: // builtin DNA length
            parser->fscope.alphabet = alph_dna();
            break;
          case 20: // builtin protein length
            parser->fscope.alphabet = alph_protein();
            break;
          default:
            html_error(parser, "The %s of motif %s has an alphabet length value %d "
                "which does not match the lengths of any builtin alphabets.\n", 
                type, name, alen);
        }
      }
    }
...
void meme_alphabet(CTX_T *ctx, const char *alphabet) {
  if ((ctx->fscope.alphabet = alph_type(alphabet, 30)) == NULL) {
...
void meme_bgfreq(CTX_T *ctx, const char *bgfreqs) {
  ...
  while (regexec_or_die("bg letter", &(ctx->re.bg_letter), substr, 4, matches, 0)) {
    ...
    if (*alph != NULL) {
      if (bg == NULL || get_array_length(bg) != alph_size_core(*alph)) {
        bg = resize_array(bg, alph_size_core(*alph));
      }
    } else {
...
void enqueue_motif(CTX_T *ctx) {
  struct mscope *mdata;
  if (ctx->mscope.motif) {
    ctx->mscope.motif->alph = alph_hold(ctx->fscope.alphabet);
