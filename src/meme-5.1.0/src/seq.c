...
SEQ_T *shuffle_seq (
  ...
  int l_copy = log(i_copy)/log(10) + 1; // number of digits in the copy number
  char *c_shuf = "_shuf_";
  int pad = strlen(c_shuf);
  int max_seq_name = MAX_SEQ_NAME - pad - l_copy;       // leave room for copy number
  ...
  sprintf(new_seq->name, "%s%s%-i", name, c_shuf, i_copy);
